...
// Per-kind demotion applied after the match tier is decided. Small enough to
// stay inside a tier (tiers are 1000 apart): a category row loses to a
// same-tier command, a menu leaf loses to the category it lives in, but a
// prefix match on a leaf still beats a keyword match on a command.
var KIND_PENALTY = {
  "omarchy-category": 100,
  window: 200,
  "omarchy-menu": 400,
  file: 600
}

var FAVORITE_BONUS = 300

// Rank entries for a query. frecencyScores: map id -> number (see
// Frecency.js scores()). limit: 0/undefined = uncapped. opts (optional):
//   { favoriteBias: false }  -> favorites get no +300 bump (for root-page
//                               sections that already list favorites apart)
// Returns [{ entry, score }] sorted best first.
function rank(entries, query, frecencyScores, limit, opts) {
  var q = norm(query)
  var rows = []
  var fs = frecencyScores || {}
  var favoriteBias = !(opts && opts.favoriteBias === false)
  for (var i = 0; i < entries.length; i++) {
    ...
    if (favoriteBias && e.favorite) s += FAVORITE_BONUS
...
if (typeof module !== "undefined") {
  module.exports = { norm: norm, words: words, acronym: acronym, prepare: prepare, fuzzyScore: fuzzyScore, rank: rank, KIND_PENALTY: KIND_PENALTY, FAVORITE_BONUS: FAVORITE_BONUS }
}
